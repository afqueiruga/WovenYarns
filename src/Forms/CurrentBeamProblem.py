...
class CurrentBeamProblem(ProblemDescription):
    ...
    def Declare_Spaces(self):
        ...
        S = FunctionSpace(self.mesh,"CG",self.order[0])
        d = {
            'S': S,
            'V': V,
    ...
    def Build_Forms(self):
        ...
        for z1,z2,weight in GPS2D:
            ...
            MLoc = weight*J0*(inner(tv,rho*dvdt) )*dx
        ...
        ContactForm = -dot(jump(tvq),
                        conditional(ge(overlap,0.0), -0.04*overlap,0.0)*jump(xr)/dist)*dc(0, metadata={"num_cells": 2,"special":"contact"})
